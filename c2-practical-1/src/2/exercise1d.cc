...
int main(int argc, char ** argv) 
{
  ...
  if (argc<4) 
  {
    cout << "Usage: "<< argv[0] << " <problem> <N> <loops>" << endl;
    return 1;
  }
  int prob = atoi( argv[1] );
  // start discretization (does not need to be const)
  int N = atoi( argv[2] );
  // number of loops for eoc computation
  ...
  for (int l=0;l<loops;++l)
  {
    ...
    // we want to add the loop number to the file name - that is not
    // simple...
    std::stringstream filename; 
    filename << "interpol1d_" << l << ".gnu";
