      ...
      const bool detailed = false )
{
  ...
  const std::size_t maxIter = std::numeric_limits<std::size_t>::max();
  return cgSolver(b.size(), A, b.raw(), x.raw(), eps, maxIter, detailed );
}
template< class T, class MATRIX >
inline int
cgSolver( const MATRIX &A, 
...
namespace Helper { // anonymous name space only so that doxygen does not add to documentation
//============================================================================
// some BLAS routines 
//============================================================================
...
template <class MATRIX>
inline int cgSolver( const unsigned int N, 
                 const MATRIX &A, 
                 const double *b, 
                 double *x, 
                 const double eps,
                 ...
                 const bool detailed ) 
{
  ...
  while ( scalarproduct(N,g,g) > err && its < maxIter )
  {
    ...
    if ( detailed )
    {
      cout<<"cgSolver "<<its<<"\t"<<sqrt(scalarproduct(N,g,g))<<endl;
    }
  ...
  if (its == maxIter)
    cout<<"FAILIUR: cgSolver "<<its<<"\t"<<sqrt(scalarproduct(N,g,g))<<endl;
  else if ( detailed)
    cout<<"cgSolver "<<its<<"\t"<<sqrt(scalarproduct(N,g,g))<<endl;
