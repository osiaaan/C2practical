#ifndef VTKOUTVF_HH_INCLUDED
#define VTKOUTVF_HH_INCLUDED
...
class OutputVF
{
  ...
  //---------------------------------------------------------------------
  template <class DF, unsigned int dim>
  struct VTKWrapper : public Dune::VTKWriter<Dune::GridPartView>::VTKFunction
  {
    ...
    {
      if (comp>=dim) return 0.;
      ElementType element_(df_.grid(), e.template subEntity<0>(0));
      double val = df_.evaluate(element_, xi)[comp];
  ...
  template <class DF>
  struct VTKWrapper<DF,1> : public Dune::VTKWriter<Dune::GridPartView>::VTKFunction
  {
    typedef typename Dune::VTKWriter<Dune::GridPartView>::VTKFunction Base;
    typedef typename Dune::GridPartView:: template Codim<0>::Entity Entity;
  ...
  void add(const DiscreteFunction<BaseFunctionSet>& discreteFunction,
           const std::string name = "Uh" )
  {
    VTKWrapper< DiscreteFunction<BaseFunctionSet>,1 >* f =
      new VTKWrapper< DiscreteFunction<BaseFunctionSet>,1 >(discreteFunction, name);
    out_.addVertexData( f );
  }
  template <class BaseFunctionSet>
  void add(const DiscreteVectorfield<BaseFunctionSet>& discreteFunction,
           const std::string name = "Uh" )
  {
    VTKWrapper< DiscreteVectorfield<BaseFunctionSet>, Dune::dimw >* f =
      new VTKWrapper< DiscreteVectorfield<BaseFunctionSet>, Dune::dimw >(discreteFunction, name );
    out_.addVertexData( f );
  }
