...
//#include "CrouzeixRaviartBasisFunctions.hh"
...

    {
      ...
      for (int qp=0; qp<quad.nop(); ++qp)
      {
        ...
        for (int k=0; k< locNrDof; ++k)
        {
          ...
          matrix[k][k] += weight * problem.lambda() * (phi[k] * phi[k]);
          // add grad of the phi
          GlobalCoordType gradientk;
          gradientk= uh.gradient(element, k, quad.point(qp) );

          matrix[k][k] += weight * ( gradientk[0]*gradientk[0] + gradientk[1]*gradientk[1] );

          // eval others
          for (int j=0; j< k; ++j )
          {
            double I = weight * (phi[k] * phi[j]);
            GlobalCoordType gradientj;
            gradientj= uh.gradient(element, j, quad.point(qp) );
             I += weight * ( gradientk[0]*gradientj[0] + gradientk[1]*gradientj[1] );
            matrix[k][j] += I;
            matrix[j][k] += I;
          }
        }
  ...
  // boundary treatment
  {
    ...
    for(ElementIteratorType it = grid.begin(); it != endit; ++it)
    {
      ...
      for ( int edge = 0; edge < element.nCorners; ++edge )
      {
          ...
          {
          for(int vx=0; vx<locNrDof; vx++)
              {
              if( element.onBoundary(edge))
                  {
                      if( uhbase.onBnd(edge, vx))
                      {
                           F.dof(element,vx)=problem.g(element[vx]);
                           uh.dof(element,vx)=problem.g(element[vx]);
                           M.setDiag( uhbase.map(element,vx) , 1);
                      }
                  }
              }
          }
